...
impl Completions {
    pub fn new(entries: Vec<String>) -> Completions {
        const COMPLETION_LINES: usize = 5;

        let mut arc_entries = Vec::new();
        for e in entries {
            arc_entries.push(Arc::new(e));
        }
    ...
    pub fn move_cursor(&mut self, offset: isize) {
        ...
        trace!("move_cursor: offset={}, index={}", offset, self.selected_index);
    }
...
impl CompletionContext {
    pub fn current_word(&self) -> &str {
        self.words.iter().nth(self.current_word_index as usize).map(|s| s.as_str()).unwrap_or("")
    }
}

/// The `Completion` Builder.
pub struct CompGen<'a> {
    entries: Vec<String>,
...
impl<'a> CompGen<'a> {
    ...
    #[inline]
    pub fn entries(mut self, entries: Vec<String>) -> CompGen<'a> {
        self.entries = entries;
        self
    }

    #[inline]
    pub fn search(mut self, query: &'a str) -> CompGen<'a> {
    ...
    pub fn build(self) -> Completions {
        let filtered_entries = self.entries; // TODO: fuzzy search
        Completions::new(filtered_entries)
    }
}
...
fn path_completion(ctx: &CompletionContext) -> Completions {
    ...
    for e in std::fs::read_dir(dir).unwrap() {
        ...
        entries.push(e.unwrap().path().to_str().unwrap().to_owned());
    }
...
pub fn extract_completion_context(user_input: &str, user_cursor: usize) -> CompletionContext {
    ...
    trace!("words={:?}, index={}, offset={}, len={}", words, current_word_index, current_word_offset, current_word_len);
